...
"玉溪市商业银行·红塔卡"]
# 传入卡号 得到银行名称
def getBankName(idCard):
	index = -1
	if (len(idCard) == 0 or len(idCard) < 16 or len(idCard) > 19):
		return "您输入的银行卡号不正确"
	# 6位Bin号
	cardbin_6 = idCard[0:6]
	for i in  range(len(bankBin)):
		if (cardbin_6 == bankBin[i]):
			index = i
	if (index != -1):
		return bankName[index]
	#8位Bin号
	cardbin_8 = idCard[0:8]
	for i in range(len(bankBin)):
...
def getbankCardNo(number):
	#银行卡号bin
	bankbin=["622402","628312","625911","621608","620714","622268","622788","621724","622168","625084","622840","622255","622757","623022","622261","622663"]
	if number ==16:#bankBin，银行的Bin号
		bankCardNo = random.choice(bankBin)+"".join(random.choice("0123456789") for i in range(10))
	else:
		bankCardNo = random.choice(bankbin) + "".join(random.choice("0123456789") for i in range(13))
	return bankCardNo
#当bankCode为空时，中国招商银行卡号 China Merchants Bank
def getCMBbankCardNo():
	cmbankbin = ["622580", "622609", "621486"]  # 宝付渠道，必须是招商，尾数4~8
	cardend = ["4", "5", "6", "7", "8"]
	cmbankCardNo = random.choice(cmbankbin) + "".join(random.choice("0123456789") for i in range(9))+"".join(random.choice(cardend))
	return cmbankCardNo
#当bankCode为空时，中国建设银行卡号 China Construction Bank
def getCCBbankCardNo():
	ccbankbin =["621700","622280"] #快付通渠道，必须是建行 #"621598"
	ccbankCardNo = random.choice(ccbankbin) + "".join(random.choice("0123456789") for i in range(13))
	return ccbankCardNo
#获取银行代码
def getbankCode(bankcardNo):
	bankCode =""
	bankname = getBankName(bankcardNo)#卡号名称
	#print(bankname)
	for key in bankdic:
		if str(bankname).__contains__(key):
			bankCode = bankdic[key]
	return bankCode
#获取银行卡号
bankAcctNo = getCCBbankCardNo() # 卡号 验证卡号是否有效luhn.is_valid("6221681421017264254")
#根据银行卡号，获取银行代码
#bankCode = getbankCode(bankAcctNo)
#print(bankAcctNo)
#print(bankCode)
